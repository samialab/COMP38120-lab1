...
STATE=`curl -u admin:${SEC} -G https://${CLUSTER_NAME}.azurehdinsight.net/templeton/v1/status | jq .status `
...
JOB_ID=`curl -u admin:${SEC} -d user.name=${LOGIN} -d jar=${JAR_NAME} -d class=${CLASS_NAME} -d arg="-input" -d arg="/input/" -d arg="-output" -d arg="/comp38120/output" -d arg="-numReducers" -d arg=${NUM_REDUCER} https://${CLUSTER_NAME}.azurehdinsight.net/templeton/v1/mapreduce/jar | jq .id`
