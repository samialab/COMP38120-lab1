...
public class BasicInvertedIndex extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        options.addOption(Option.builder(INPUT).argName("path").hasArg().desc("input path").build());
        options.addOption(Option.builder(OUTPUT).argName("path").hasArg().desc("output path").build());
        options.addOption(Option.builder(NUM_REDUCERS).argName("num").hasArg().desc("number of reducers").build());
        ...
        DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern("yyyy-mm-dd'T'HH-mm-dd");
        String currentTime = LocalDateTime.now().format(dateTimeFormatter);
        String outputPath = cmdline.getOptionValue(OUTPUT) + "-" + currentTime;
